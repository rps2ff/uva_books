...
<table class="table mt-5">
  <thead class="thead text-primary">
    <tr class="mt-3 bg-light">
      <th><h3 class="pt-3 ml-5">Sold Items</h3></th>
      <th><h3 class="pt-3 ml-5"></h3></th>
    </tr>
  </thead>
  <tbody>
	  <c:forEach var="item" items="${sold_items}">
	 
	  <tr>
	      <td>
		      <h4 class="ml-5"><c:out value="${item.title}" /></h4>
		      <p class="ml-5">Buyer: <c:out value="${item.buyername}" /></p>
		      <p class="ml-5">Method Of Payment: <c:out value="${item.method_of_payment}" /></p>
	      </td>    
