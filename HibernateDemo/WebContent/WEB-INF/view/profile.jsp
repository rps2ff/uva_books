...
<table class="table mt-5">
  <thead class="thead text-primary">
    <tr class="mt-3 bg-light">
      <th><h3 class="pt-3 ml-5">Profile</h3></th>
      <th><h3 class="pt-3 ml-5"></h3></th>
    </tr>
  </thead>
  <tbody>
	  <c:url var="updateLink" value="/customer/showUpdateProfileForm">
		<c:param name="userid" value="${currentuser.id}" />
	  </c:url>
	
	  <tr>
	      <td>
		      <h4 class="ml-5"><c:out value="${currentuser.firstName}" /> <c:out value="${currentuser.lastName}" /></h4>
		      <p class="ml-5">Email: <c:out value="${currentuser.email}" /></p>
		      <p class="ml-5">Street: <c:out value="${currentuser.street}" /></p>
		      <p class="ml-5">City: <c:out value="${currentuser.city}" /></p>
		      <p class="ml-5">State: <c:out value="${currentuser.state}" /></p>
		      <p class="ml-5">Phone Number: <c:out value="${currentuser.phoneNumber}" /></p>
	      </td>    
	      <td>
			<a href="${updateLink}" class=" btn btn-outline-warning my-2 my-sm-0">Update</a>
			<a href="/HibernateDemo/customer/viewMyOrders"" class=" btn btn-outline-primary my-2 my-sm-0">My Orders</a>
			<a href="/HibernateDemo/customer/viewSoldItems"" class=" btn btn-outline-primary my-2 my-sm-0">Sold Items</a>
	      </td>       
	  </tr>
  </tbody>
  </table>
